// Falla el build si el análisis público destacado de la landing (link "Mirá un
// análisis real que corrió Nichalo, sin registrarte →", en PricingSection y al
// pie de la sección #ejemplo) no resuelve para un visitante SIN sesión.
//
...
//
// Ese link es ahora la ÚNICA afirmación verificable de la landing, así que
// este check importa más que antes, no menos: la card de arriba se presenta
// como ejemplo, pero acá se promete un análisis real y abrible.
//
// Lo que este script YA NO hace (21/9): comparar los números de la landing
// contra los del registro. La card pasó a ser un informe de muestra
// construido a mano y etiquetado como tal (ver lib/ejemplo-landing.ts), así
// que no tiene por qué coincidir con este análisis — son dos cosas distintas
// y la landing las anuncia distinto.
//
...
async function main() {
  ...
  const url = `${SUPABASE_URL}/rest/v1/analyses?id=eq.${FEATURED_RESULT_ID}&select=id,producto,score,veredicto`;
