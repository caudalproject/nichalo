 ...
 * porque el momento en que alguien quiere vigilar un nicho es justo despues de
 * ver que el nicho le interesa — no en un menu aparte.
 *
 * Solo se muestra al dueno del analisis: un link compartido lo puede abrir
 * cualquiera (decision del TAB 4), pero vigilar es una accion de cuenta.
 */
export function BotonVigilar({ analysisId }: { analysisId: string }) {
    ...
    return (
      <div className="rounded-xl border border-[#16A34A]/20 bg-[#16A34A]/5 px-4 py-3">
        <p className="text-sm font-medium text-gray-900">
          Listo, lo estás vigilando.
        </p>
        <p className="mt-0.5 text-xs text-gray-500">
          Falta una segunda medición para poder comparar.{" "}
          <a href="/vigilancia" className="font-medium text-[#16A34A] hover:underline">
            Ver mis nichos
          </a>
        </p>
      </div>
    ...
    <div>
      ...
      >
        <span className="text-sm font-medium text-gray-900">
          {estado === "enviando" ? "Agregando…" : "Vigilar este nicho"}
        </span>
